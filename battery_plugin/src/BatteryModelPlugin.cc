                           ...
                           sdf::ElementPtr _sdf)
{
  ...
  else
  {
    ...
    this->batteryFilePath /= "scores";
    this->batteryFilePath /= this->world->GetName() + ".score";
  }
...
void BatteryModelPlugin::OnUpdate()
{
  ...
  if (this->battCharge > 0.0)
  {
    ...
    this->leftWheel->SetVelocity(0, leftVelDesired);
    this->rightWheel->SetVelocity(0, rightVelDesired);

    this->leftWheel->SetMaxForce(0, this->torque);
    this->rightWheel->SetMaxForce(0, this->torque);

    // temporary, assumed 100% charge; 0.005% per update
    this->battCharge = this->battCharge - 0.005;

    if (this->battCharge < 50)
        gzwarn << "Battery: " << this->battCharge  << std::endl;
  }
  else
  {
    // abrupt, temporary
    this->leftWheel->SetVelocity(0, 0);
    this->rightWheel->SetVelocity(0, 0);
  }
