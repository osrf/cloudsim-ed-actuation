                           ...
                           sdf::ElementPtr _sdf)
{
  ...
  cmd_vel_subscriber_ = this->rosNode->subscribe<geometry_msgs::Twist>("/set_vel", 1, boost::bind(&BatteryModelPlugin::cmdVelCallback, this, _1));

  cmd_vel_throttle_publisher_ = this->rosNode->advertise<geometry_msgs::Twist>("/dd/cmd_vel", 1);
...
void BatteryModelPlugin::SetParameters()
{
  ...
  {    
    ...
    {
        this->ratedMotorVoltage = 24.0;
        this->torqueConstant = 7.9;
        this->maxRPM = 1033;
...
void BatteryModelPlugin::OnUpdate()
{
  ...
  if (!fmod(simTime.Double(),10))
  {
    ...
    if (startDischarge)
        this->battCharge = this->battCharge - this->dischargeRate - 0.5;
